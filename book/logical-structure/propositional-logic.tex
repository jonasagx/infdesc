...
Como nosso objetivo era uma expressão na forma `if\dots{}then\dots{}', fazia sentido começar assumindo a declaração `if' e usar essa suposição para provar a declaração `then'. Como tal, a primeira coisa que escrevemos em nossa prova foi:
\begin{quote}
...
The \textbf{biconditional} operator is the logical operator ⇔\Leftrightarrow \inlatex{Leftrightarrow}\lindexmmc{Leftrightarrow}{⇔\Leftrightarrow}, defined by declaring p⇔qp \Leftrightarrow q to mean (p⇒q)∧(q⇒p)(p \Rightarrow q) \wedge (q \Rightarrow p). The expression p⇔qp \Leftrightarrow q represents `pp if and only if qq'.
\end{definition}
