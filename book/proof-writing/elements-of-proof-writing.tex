...
\begin{quote}
Let X, Y and Z be sets, let \xtremph{F be a function from X to Y} and \xtremph{G be a function} \xtremph{from Y to Z}, and assume that F and G are injective. Let \xtremph{A and B be elements} \xtremph{of X} and assume that \xtremph{G F of A is equal to G F of B}. Then \xtremph{G of F of A equals} \xtremph{G of F of B} by definition of \xtremph{function composition}, \xtremph{so} \, \xtremph{F of A equals F of B} by injectivity of G, \xtremph{and so} \, \xtremph{A equals B} by injectivity of F. Hence \xtremph{G F} is injective, as required.
\end{quote}
...
\begin{quote}
Let \xtremph{N be an integer} and suppose that N is even. Then N equals two K for some \xtremph{integer K}.
\end{quote}
...
\begin{itemize}
\item For the upper-case letters that are identical to a letter in the Latin alphabet, use the \texcodebs{mathrm} command together with the Latin letter. For example, upper-case \textit{rho} can be input as \texcodebs{mathrm\{P\}}, even though \textit{rho} corresponds phonemically with the Latin letter R.
