...
Nós vamos evitar nos referir ao conjunto unversersal \mathcal{U}\mathcal{U} sempre que possível,mas isto estará sempre em segundo plano. Isso é conveniente porque nós não precisamos mais nos preocupar sobre o domínio de discurso de variáveis libre (como fizemos em \Cref{defPredicate}),então podemos abreviar `\forall x \in \mathcal{U},\, p(x)\forall x \in \mathcal{U},\, p(x)' por `\forall x,\, p(x)\forall x,\, p(x)', e `\exists x \in \mathcal{U},\, p(x)\exists x \in \mathcal{U},\, p(x)' por `\exists x,\, p(x)\exists x,\, p(x)'.
...
Uma maneira é simplesmente fornecer uma \textbf{lista} dos elementos do conjunto. Para especificar que a lista denota um conjunto, colocamos a lista entre $\{$curly brackets$\}$\nindex{set}{$\{ \cdots \}$}{definir notação
} \inlatex{\{,\textbackslash{}\}}\lindexmmc{\{\dots\textbackslash{}\}}{$\{\dots\}$}. Por exemplo, o seguinte é uma especificação de um conjunto $X$, cujos elementos são os números naturais entre $0$ e $5$ (inclusive):
\[ X = \{ 0, 1, 2, 3, 4, 5 \} \]

\textbf{Listas implícitas.}\index{notação de lista implícita}
...
\begin{exercise}
Seja XX um conjunto. Mostre que ∅∈P(X)\varnothing \in \mathcal{P}(X) e X∈P(X)X \in \mathcal{P}(X).
\end{exercise}

\begin{exercise}
Escreva os elementos de P(∅)\mathcal{P}(\varnothing), P(P(∅))\mathcal{P}(\mathcal{P}(\varnothing)) e P(P(P(∅)))\mathcal{P}(\mathcal{P}(\mathcal{P}(\varnothing))).
\end{exercise}
...
U⊆X⇔U∈P(X) U \subseteq X \quad \Leftrightarrow \quad U \in \mathcal{P}(X) 
...
\begin{itemize}
\item ∅⊆∅\varnothing \subseteq \varnothing means ∀x∈∅,x∈∅\forall x \in \varnothing,\, x \in \varnothing; mas proposições da forma ∀x∈∅,p(x)\forall x \in \varnothing,\, p(x) são sempre verdadeiras, como discutido em \Cref{exEverythingIsTrueOfElementsOfEmptySet}.
...
\begin{enumerate}[(a)]
\item P(∅)∈P(P(∅))\mathcal{P}(\varnothing) \in \mathcal{P}(\mathcal{P}(\varnothing));
\item ∅∈{{∅}}\varnothing \in \{ \{ \varnothing \} \};
\item {∅}∈{{∅}}\{ \varnothing \} \in \{ \{ \varnothing \} \};
\item P(P(∅))∈{∅,{∅,{∅}}}\mathcal{P}(\mathcal{P}(\varnothing)) \in \{ \varnothing, \{ \varnothing, \{ \varnothing \} \} \}.
\end{enumerate}
