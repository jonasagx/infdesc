...
A expressão x∈Xx \in X\nindex{in}{∈\in}{element} \inlatex{in}\lindexmmc{in}{∈\in} denota a afirmação de que xx é um elemento de X X; escrevemos x∉Xx \not \in X \inlatex{not\textbackslash{}in}\lindexmmc{not}{\not\in, \not\equiv, \dots\not\in, \not\equiv, \dots} para significar \neg (x \in X)\neg (x \in X), ou seja, xx não é um elemento de XX.
\end{definition}
...
Uma maneira é simplesmente fornecer uma \textbf{lista} dos elementos do conjunto. Para especificar que a lista denota um conjunto, colocamos a lista entre \{\{colchetes\}\}\nindex{set}{\{ \cdots \}\{ \cdots \}}{set notation} \inlatex{\{,\ barra de texto{}\}}\lindexmmc{\{\dots\textbackslash{}\}}{\{\dots\}\{\dots\}}. Por exemplo, o seguinte é uma especificação de um conjunto XX, cujos elementos são os números naturais entre 00 e 55 (inclusive):
 X = \{ 0, 1, 2, 3, 4, 5 \}  X = \{ 0, 1, 2, 3, 4, 5 \} 

\textbf{Listas implícitas.}\index{notaçao de listas impllícitas}
...
Em geral, as listas implícitas podem ser ambíguas, portanto, na prática, elas são evitadas, a menos que a lista implícita seja muito simples, como um conjunto de números consecutivos como {3,4,…,9}\{3, 4, \dots, 9 \}. Na verdade, muitos conjuntos nem sequer podem ser listados desta forma.

Para contornar isso, podemos usar \textit{notação set-builder}, que é um meio de especificar um conjunto em termos das propriedades que seus elementos satisfazem. Dado um conjunto XX, o conjunto de elementos de XX que satisfazem alguma propriedade p(x)p(x) é denotado
{x∈X∣p(x)} \{ x \in X \mid p(x) \} 
A barra `∣\mid' \inlatex{mid}\lindexmmc{mid}{∣\mid} separa o nome da variável da fórmula que eles tornam verdadeira --- alguns autores usam dois pontos (como em {x∈X:p(x)}\{ x \in X : p(x) \}).

O conjunto {x∈X∣p(x)}\{ x \in X \mid p(x) \} é lido em voz alta como `o conjunto de x∈Xx \in X tal que p(x)p(x)', mas cuidado --- nem a barra `∣\mid' nem os dois pontos `::' significam `tal que' em outros contextos.
...
{n∈Z∣n é par} \{ n \in \mathbb{Z} \mid n \text{ é par} \} 
Para efeito de comparação, o conjunto de todos os números naturais pares pode ser escrito como
{n∈N∣n é par}={0,2,4,6,…} \{ n \in \mathbb{N} \mid n \text{ é par} \} = \{ 0, 2, 4, 6, \dots \} 
Observe que −6-6 é um elemento do primeiro conjunto, mas não do último conjunto, já que −6-6 é um número inteiro, mas não é um número natural.

Observe ainda que a expressão
{n∈Q∣n é par} \{ n \in \mathbb{Q} \mid n \text{ é par} \} 
não tem sentido, uma vez que não definimos uma noção de “igualdade” para números racionais.
\end{example}

\begin{strategy}
Seja XX um conjunto e p(x)p(x) uma fórmula lógica com variável livre x∈Xx \in X. Para provar a∈{x∈X∣p(x)}a \in \{ x \in X \mid p(x) \}, basta provar a∈Xa \in X e que p(a)p(a) é verdadeiro.
\end{strategy}
